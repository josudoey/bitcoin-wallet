...
const Wallet = function (opts) {
  ...
  this.mnemonicStrength = parseInt(config.mnemonic.strength) || 128
}
...
Wallet.prototype.initSeed = function () {
  ...
  if (!mnemonic) {
    mnemonic = bip39.generateMnemonic(this.mnemonicStrength)
    console.log('====save seed mnemonic somewhere safe and secret====')
...
Wallet.prototype.showSeed = function () {
  const walletPath = config.wallet.path
  const exists = fs.existsSync(walletPath)
  if (!exists) {
...
Wallet.prototype.changePassword = function () {
  ...
  const newPassword = Wallet.getNewPassword()
  const newCiphertext = sjcl.encrypt(newPassword, mnemonic)
  const backupPath = walletPath + '.old'
  console.log(`backup to ${backupPath}`)
  fs.renameSync(walletPath, backupPath)
  fs.writeFileSync(walletPath, newCiphertext)
  console.log(`change success ${walletPath}`)
